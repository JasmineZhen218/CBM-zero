...
from data_utils import load_labels, get_data, process_cub_annotations
...
def train_cbm_zero(args):
    ...
    print(
        "Explaining the classification model: {} on dataset: {} with concept bank: {}".format(
            args.black_box_model_name, args.data_name, args.concept_set_source
        )
    )
    # load class names
    ...
    if os.path.exists(cx_train_path) and os.path.exists(cx_val_path):
        ...
        cx_train = torch.load(
            cx_train_path, map_location=args.device, weights_only=True
        )
    ...
    if os.path.exists(bb_features_train_path) and os.path.exists(bb_features_val_path):
        print(
            "Load saved hidden space features from: {} and {}".format(
        ...
        print(
            "Extract hidden space features of the black-box model: {}".format(
    ...
    logits_val = bb_features_val @ proj_activation2class.T + proj_activation2class_bias
    ...
    for i in range(args.n_iter):
        ...
        regul_loss = (
            args.alpha * torch.norm(proj_concept2activation, p=1)
            + (1 - args.alpha) * torch.norm(proj_concept2activation, p=2) ** 2
        ) / d
        ...
        if (i + 1) % 10 == 0:
            with torch.no_grad():
                ...
                loss_val = mse_loss_val + args.lambd * regul_loss
                print(
                    "iter = {}\n\
    ...
    print(
        "Mapping from hidden space to concept space is saved at: {}".format(
            projection_path
        )
    )
    # construct the CBM
    print("Derive the projection from concepts to classes")
    proj_activation2concept = torch.load(
        projection_path, map_location=args.device, weights_only=True
    )
